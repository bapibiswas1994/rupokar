<x-admin.table>
    ...
    <x-slot name="thead">
        <tr role="row">
            ...
            <th tabindex="0" aria-controls="kt_table_1" rowspan="1" colspan="1" style="width: 20%;"
                aria-label="Company Email: activate to sort column ascending">Ratings <i
        ...
        <tr class="filter">
            ...
            <th>
                <select class="form-control form-control-sm form-filter kt-input" wire:model.defer="searchRating"
    ...
    <x-slot name="tbody">
        @forelse($testimonial as $item)
            <tr role="row" class="odd">
                <td>
                    <a class="kt-link" href="{{ route('users.show',$item->user->id) }}">{{ $item->user->full_name }}</a>
                </td>
                <td>{{ $item->getRatings($item->ratings) }}</td>
                <td>{{ $item->description }}</td>
                <td class="align-center">
                    <span class="kt-badge  kt-badge--{{ $item->status == 1 ? 'success' : 'warning' }} kt-badge--inline kt-badge--pill cursor-pointer"
                        wire:click="changeStatusConfirm({{ $item->id }})">
                        {{ $item->status == 1 ? 'Active' : 'Inactive'}}
                    </span>
                </td>
                <x-admin.td-action>
                    <a class="dropdown-item" href="{{ route('testimonial.edit', ['testimonial' => $item->id]) }}"><i
    ...
    <x-slot name="showingEntries">
        Showing {{ $testimonial->firstitem() ?? 0 }} to {{ $testimonial->lastitem() ?? 0 }} of {{ $testimonial->total() }} entries
    </x-slot>
</x-admin.table>