<x-admin.table>
    ...
    <x-slot name="thead">
        <tr role="row">
            <th tabindex="0" aria-controls="kt_table_1" rowspan="1" colspan="1" style="width: 10%;"
                aria-sort="ascending" aria-label="Agent: activate to sort column descending">ID <i
            ...
            <th tabindex="0" aria-controls="kt_table_1" rowspan="1" colspan="1" style="width: 15%;"
                aria-sort="ascending" aria-label="Agent: activate to sort column descending">Vendor Name <i
                    class="fa fa-fw fa-sort pull-right" style="cursor: pointer;" wire:click="sortBy('first_name')"></i>
            </th>
            <th tabindex="0" aria-controls="kt_table_1" rowspan="1" colspan="1" style="width: 15%;"
            ...
            <th tabindex="0" aria-controls="kt_table_1" rowspan="1" colspan="1" style="width: 15%;"
                aria-label="Company Email: activate to sort column ascending">Feedback Date <i
    ...
    <x-slot name="tbody">
        @forelse($feedbacks as $feedback)
            <tr role="row" class="odd">
                <td>{{ $feedback->id }}</td>
                <td><a class="kt-link" href="{{ $feedback->user_id }}">{{ $feedback->user->full_name }}</a></td>
                <td><a class="kt-link" href="{{ $feedback->vendor_id }}">{{ $feedback->vendor->full_name }}</a></td>
                <td>{{ $feedback->subject }}</td>
                <td>{{ \Carbon\Carbon::parse($feedback->created_at)->format('d/m/Y H:i:s A')}}</td>
                <td class="align-center">
                    <span
                        class="kt-badge  kt-badge--{{ $feedback->active == 1 ? 'success' : 'warning' }} kt-badge--inline kt-badge--pill cursor-pointer"
                        wire:click="changeStatusConfirm({{ $feedback->id }})">
                        {{ $feedback->active == 1 ? 'Active' : 'Inactive' }}
                    </span>
                </td>
                <x-admin.td-action>
                    <a class="dropdown-item" href="{{ route('feedback.show', ['feedback' => $feedback->id]) }}"><i
                        class="la la-eye"></i> Show</a>
                    <a class="dropdown-item" href="{{ route('feedback.edit', ['feedback' => $feedback->id]) }}"><i
                            class="la la-edit"></i> Edit</a>
                    <button href="#" class="dropdown-item" wire:click="deleteAttempt({{ $feedback->id }})"><i
    ...
    <x-slot name="showingEntries">
        Showing {{ $feedbacks->firstitem() ?? 0 }} to {{ $feedbacks->lastitem() ?? 0 }} of {{ $feedbacks->total() }}
        entries
    </x-slot>
</x-admin.table>