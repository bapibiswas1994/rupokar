...
class CategoryHelper
{
    ...
    public static function getCategories($parent = 0, $notIn = array(0))
    {
        ...
        $cats = Category::select(DB::raw("CONCAT('$url',cat_image) AS imageurl"),
            'id', 'parent_id', 'cat_title', 'cat_slug', 'cat_image', 'cat_status',
            'cat_short_desc', 'cat_long_desc', 'meta_keywords', 'meta_description',
            'hsncode', 'is_service_available', 'service_installation_charges', 'is_rms_available', 'rms_service_charges',
            'gst_rate', 'commision_rate', 'product_listing_rate', 'delivery_rate'
        )
            ->where('cat_status', 1)
